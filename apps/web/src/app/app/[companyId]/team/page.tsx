...
export default function TeamPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      createPerson(companyId, {
        kind,
        display_name: name.trim(),
        role_type: roleType,
        specialty: specialty.trim() || undefined,
        ai_profile_id: kind === "ai_agent" && aiProfileId ? aiProfileId : undefined,
      }),
    onSuccess: () => {
      ...
      {showForm && (
        <Card className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {/* AI profile selector — only for AI agents */}
            {kind === "ai_agent" && (
              <div className="col-span-2">
                <label className="text-xs text-zinc-500 mb-1.5 block">
                  AI Profile
                ...
                {!aiProfiles?.length ? (
                  <p className="text-xs text-zinc-600">
                    No AI profiles configured. Set one up in Settings first.
                  </p>
                ) : (
                  <select
                    value={aiProfileId}
